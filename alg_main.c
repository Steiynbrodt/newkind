 ...
 * alg_main.c
 *
 * Allegro-version main game handler.
 * This version has been adapted to be Allegro 5 compatible.
 */
...
/* Allegro 4 header removed – all direct Allegro calls are gone */
/* #include "allegro.h" */
...
int climbing;
int game_paused;
int have_joystick;  /* joystick disabled in this A5 port, kept for compatibility */

int find_input;
char find_name[20];
/* Replacement for Allegro 4 get_filename() */
static char *get_filename(const char *path)
{
    ...
    if (slash1 && slash1 > last)
        last = slash1 + 1;
    if (slash2 && slash2 > last)
        last = slash2 + 1;

    return (char *)last; /* cast is fine, caller treats it as read-only */
}
 ...
 * Allegro 5: no xor_mode – just draw normal lines.
 * Erasing is handled by redrawing the chart when the cross moves.
 */
...
void run_escape_sequence(void)
{
    ...
    for (i = 0; i < 90; i++)
    {
        ...
        gfx_display_centre_text(358, "Escape pod launched - Ship auto-destuct initiated.",
                                120, GFX_COL_WHITE);
...
void handle_flight_keys(void)
{
    ...
    /* Joystick block removed in Allegro 5 minimal port – have_joystick stays 0 */
...
void load_commander_screen(void)
{
    ...
    if (rv)
    {
        ...
        gfx_update_screen();
        /* Allegro 4 readkey() replaced by our A5-compatible kbd_read_key() */
        kbd_read_key();
        return;
    }
...
void run_first_intro_screen(void)
{
    ...
    /* MIDI disabled in this A5 port (Option A: silence) */
...
/*
 * Allegro init – in Allegro 5 minimal port this does not call
 * any Allegro functions anymore. All system init happens in gfx/sound/keyboard.
 */

void initialise_allegro(void)
{
    have_joystick = 0;   /* joystick disabled in this port */
}
...
int main(void)
{
    ...
    while (!finish)
    {
        ...
        while (!game_over)
        {
            ...
            /* Crosshair rendering – no xor_mode in Allegro 5, so we
               redraw the chart and draw a fresh cross when position changes. */
